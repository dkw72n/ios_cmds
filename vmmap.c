...
void macosx_debug_regions (task_t task, mach_vm_address_t address, int max)
{
  ...
  for (;;)
  {
    ...
    if (!done)
    {
      ...
      printf("[%d] address[out] = %p, size=%x\n", __LINE__, address, size);
    ...
    if (print)
    {
      ...
      prev_address = address;
      prev_size = size;
      printf("[%d] prev_addr = %p\n", __LINE__, prev_address);
      memcpy (&prev_info, &info, sizeof (vm_region_basic_info_data_t));
      nsubregions = 1;
...
main(int argc, char **argv)
{
  ...
  macosx_debug_regions (task, addr, 10);
